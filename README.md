...
7. Finally, we can produce the count estimate(s) using `F_hat, CI = estimator.estimate(regions)`. `F_hat` are the count estimates $\hat{F}(S_i)$ for all subregions, such that `size(regions) == size(F_hat)`. `CI` returns the confidence intervals per count estimate.
